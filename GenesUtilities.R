# Single Graph V2 -------------------------------------------------------------
singleGraphV2<-function(folderPath,w=10,h=4,lib=FALSE, color = TRUE){
  ...
  for (path in paths){
    ...
    postscript(file = out1,colormodel="rgb",onefile=FALSE, horizontal=FALSE,paper = "special",width=w, height=h) 
    ...
    g3<-xyplot(nel ~ fg | fs, tricluster,
               main=tforctag,
               xlab= gentag,
               ylab= eltag, 
               groups = t,
    ...
    show(paste (path," --> [",length(levels(fg)),",",length(levels(fs)),",",length(levels(ft)),"] printed"))
...
# Single Graph -------------------------------------------------------------
singleGraph<-function(folderPath,w=10,h=4,lib=FALSE, color = TRUE){
...
# Experiment Graph --------------------------------------------------------
expGraph<-function(folderPath,w=8,h=5,lib=FALSE, color = TRUE){
...
# Build Normalized --------------------------------------------------------
buildTriGenNormalalized<-function(geoPlat,datasetAffPath,samples,times,affFolderPath,outPath){
...
# Build Gene File ---------------------------------------------------------
buildGeneFile<- function(inputPath,outputPath,geoPlat,geoAcc,lib=FALSE){
...
# Get Gene Cluster --------------------------------------------------------
getGeneCluster <- function(clusterAffGenes,geoPlatform,whitoutRepeats = FALSE) {
  l <- c()
...
# Get GEO platform --------------------------------------------------------
getGEOplatform <- function(GEOaccession,matrix=T,lib=FALSE){
