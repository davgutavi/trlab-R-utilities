...
paintSolutionsCombo <-
  function(solutions,
    ...
    for (sol in solutions) {
      ...
      for (si in levels(fs)) {
        conditions <-
          append(conditions, as.character(samplesList[as.numeric(si) + 1]))
      }
...
cell_pattern_plot_list <-
  function(inputSolPath,
           xl = "Features",
           yl = "Occurrences") {
    ...
    paths <- getGSTtags(datasetInfo)
    features_tags <-
      as.vector(read.table(paths["times"], sep = "\n")$V1)
    res <- cell_pattern_plot_list_aux (solutions, features_tags, xl, yl)
    return (res)
  }

cell_pattern_plot_list_aux <-
  function(solutions,
           features_tags,
           xl = "Features",
           yl = "Occurrences") {
    ...
    for (tri in solutions) {
      print(paste0("Plotting solution ", i, ". Features = ", toString(levels(factor(
        tri$fl
      )))))
...
buildTimeSeriesPlots <- function(inputSolPath, yl = "") {
  ...
  for (tri in solutions) {
    print(paste0("Plotting soluion ", i, ". Time points = ", toString(levels(factor(
      tri$t
    )))))
...
# Precision agriculture: Spatio-temporal analysis ----
buildTimeSeriesPlotsNormalized <-
  function(inputSolPath,
           yl = "",
           xl = "") {
    ...
    for (tri in solutions) {
      ...
      tri$norm <-
        (((tri$el - mind) * (newRange)) / oldRange) + newMin # Formula cambiar rango eje y
...
# Zonification: Heatmaps ----
aux_insert_cluster_in_frame <- function(cluster, frame) {
  for (i in c(1:nrow(cluster))) {
    cv <- frame[which(frame$x == cluster[i, ]$x &
                        frame$y == cluster[i, ]$y), 3]
    if (length(cv) > 0) {
      if (cv != 0) {
        frame[which(frame$x == cluster[i, ]$x & frame$y == cluster[i, ]$y), 3] =
          -1
        frame[which(frame$x == cluster[i, ]$x &
                      frame$y == cluster[i, ]$y), 4] = "#FFFFFF"
      }
      else{
        frame[which(frame$x == cluster[i, ]$x &
                      frame$y == cluster[i, ]$y), 3] = cluster[i, ]$v
        frame[which(frame$x == cluster[i, ]$x &
                      frame$y == cluster[i, ]$y), 4] = cluster[i, ]$c
      }
    }
  }
...
plot_clusters_general_valid_small <-
  function(solutions, totals, bias = 25) {
    ...
    for (i in c(1:length(solutions))) {
      ...
      general_clusters[[i]] <-
        cbind(unique(data.frame(x = solutions[[i]]$s, y = solutions[[i]]$g)), v =
                i, c = paleta[i])
    ...
    for (i in c(1:(Xsize - 1))) {
      general_frame <-
        rbind(general_frame, data.frame(
          x = i,
          y = seq(0, Ysize - 1),
          v = 0,
          c = "#000000"
    ...
    gr1 <-
      aux_plot_frame(general_frame,
                     names(general_clusters),
                     ocr,
                     paleta,
                     "General")
    gr2 <-
      aux_plot_frame(valid_frame, names(valid_clusters), ocr, paleta, "Valid")
    gr3 <-
      aux_plot_frame(small_frame, names(small_clusters), ocr, paleta, "Small")
...
zonification_plot_list <-
  function(solPath, totPath, Xsize, Ysize, title) {
    ...
    for (solution in solutions) {
      ...
      for (j in c(1:nrow(cluster))) {
        ...
        gdata[which(gdata$x == cluster[j, ]$x &
                      gdata$y == cluster[j, ]$y), 3] = tv
        gdata[which(gdata$x == cluster[j, ]$x &
                      gdata$y == cluster[j, ]$y), 4] = l
        explored[which(explored$x == cluster[j, ]$x &
                         explored$y == cluster[j, ]$y), 3] = tv
        explored[which(explored$x == cluster[j, ]$x &
                         explored$y == cluster[j, ]$y), 4] = l
      }
      
      sum <- cluster_ocurrences_value(solution, totals)
      grtitle <-
        paste0("Cluster #", map_index, " Total occurrences = ", sum)
    ...
    explored_title <-
      paste0("Total explored zone, total occurrences = ",
             sum(explored$value))
...
cluster_occurrences_plot_aux <- function(solutions, totals) {
  ...
  for (solution in solutions) {
    ...
    summation <-
      rbind(summation, data.frame(id = paste0(
        "#", str_pad(id, padn, "left", pad = "0")
      ), sum = sum))
    id <- id + 1
  }
...
get_coordinate_files <- function(solPath, outputFolder) {
  ...
  rootFileName <-
    paste0(outputFolder, "/", strsplit(tail(strsplit(solPath, "/")[[1]], 1), ".sol")[[1]][1])
...
save_coordinate_dfs <- function(solutions, rootFileName) {
  ...
  for (solution in solutions) {
    write.table(
      unique(data.frame(x = solution$s, y = solution$g)),
      sep = ";",
      file = paste0(
        rootFileName,
        "_",
